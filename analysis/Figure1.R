...
for (i in seq_along(sims)) {
  ...
  percP.ind.comp.la <- round(as.numeric(median(percP.ind.la)), 3)

  percP.ind.t <- as.numeric(colMeans(tail(sim$epi$percPrEP.ind.t, 52)))
  percP.ind.comp.t <- round(as.numeric(median(percP.ind.t)), 3)
  ...
  new.df <- data.frame(scen = sims[i],
                       percP.la = percP.ind.comp.la,
                       percP.t = percP.ind.comp.t,
                       prev = prev,
                       ir = ir,
                       pia = pia)
